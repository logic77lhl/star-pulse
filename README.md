...
| 板块 | 内容 |
|---|---|
| 概览卡片 | 项目名单数、累计追踪数、覆盖语言数、快照天数 |
| **项目名单**（默认展开） | **全部追踪仓库**，含星数 / Fork / 语言 / 当日新增 / 简介；支持按项目名与简介搜索、按语言筛选、按星数·新增·Fork·创建时间排序 |
...
项目名单是**服务端全量渲染**的（不截断、不分页），搜索/筛选/排序是纯 DOM 操作，
不依赖任何库 —— 所以停用 JS 也仍是一份完整可读的名单。
