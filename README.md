...
版面取向是**概览卡片 + 两个默认收起的折叠块**。项目名单是主角，但它也默认收起 ——
一次铺开 800 行没人看得下去，所以落地页先给一块可点的「项目名单（共 N 个）」，
需要时再展开；趋势作为佐证，同样收在折叠块里。

| 板块 | 内容 |
|---|---|
| 概览卡片 | 项目名单数、中文简介覆盖率、累计追踪数、覆盖语言数、快照天数 |
| **项目名单**（默认收起） | 展开后是**全部追踪仓库**，含星数 / Fork / 语言 / 当日新增 / **中文类目** / **中文简介**；支持按项目名与简介搜索、按类目或语言筛选、按星数·新增·Fork·创建时间排序、按每屏 50 / 100 / 300 / 全部切换显示条数。展开后默认只显示前 100 条 |
...
两个折叠块共用 `details.fold` 这套样式，类名**不要绑定具体板块** ——
否则下次改版很容易只改了一边。回归测试会同时校验两块的存在、默认收起、以及前后顺序。
